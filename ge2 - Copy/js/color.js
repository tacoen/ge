TaUIFunc.prototype.csscolor = function() {
  ...
  const generateCSSStyles = () => {
    ...
    cssStyles += '}';
    cssOutput.textContent = cssOutput.innerHTML = cssStyles;
  };
...
TaUIFunc.prototype.createColorInput = function(id, label, value) {
  ...
  input.type = 'color', input.id = id, input.value = value;
  const inputLabel = document.createElement('label');
  inputLabel.for = id, inputLabel.textContent = label;
  inputContainer.appendChild(input), inputContainer.appendChild(inputLabel);
